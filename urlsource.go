...
type subURLSourceProvider interface {
	UpdateUrlSub(ctx context.Context, urlSubs []string, fetchTimeout int) ([]byte, error)
}
...
func (s *subURLSource) UpdateUrlSub(ctx context.Context, urlSubs []string, fetchTimeout int) ([]byte, error) {
	...
	for _, sub := range urlSubs {
		...
		st.Go(func() stream.Callback {
			client := fetcher.NewClient(fetchTimeout)
